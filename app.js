...
function select(id){current=id;const f=files.find(f=>f.id===id);$('name').value=f?.name||'';$('editor').value=f?.content||'';for(const key of ['name','editor','save','download'])$(key).disabled=!f;$('language').textContent=f?ext(f.name).toUpperCase():'';$('saved').textContent='';render();list();runState();}
...
$('home').onclick=()=>{filter='all';$('search').value='';document.querySelector('[data-filter="all"]').click();};$('clear').onclick=()=>$('output').textContent='';
select(files[0]?.id);

// Public repository notes with local draft protection. No repository write credentials are used.
$('save').textContent='保存草稿';
document.querySelector('aside .hint').textContent='编辑仅保存在当前浏览器。下载后请手动上传仓库 notes 目录发布。';
