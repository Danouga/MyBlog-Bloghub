...
function runState(){$('run').disabled=busy||!connected||ext($('name').value)!=='py';}
...
function environments(items){$('environment').replaceChildren();for(const item of items){const option=document.createElement('option');option.value=item;option.textContent=item;$('environment').append(option);}}
$('connect').onclick=async()=>{try{const data=await api('/environments',{});environments(data.environments);connected=true;$('connection').textContent='● 本地 Python 已连接';$('configStatus').textContent='连接成功，可以选择环境并运行代码。';}catch(e){connected=false;$('connection').textContent='● 本地 Python 未连接';$('configStatus').textContent='连接失败：'+e.message+'。请确认服务启动、密钥正确，并允许浏览器访问本地网络。';}runState();};
